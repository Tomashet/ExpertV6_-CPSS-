...
class MPCLikeSafetyShield:
    ...
    def filter_action(self, env, action, cur_ctx_id: int) -> Tuple[object, Dict]:
        if self.no_mpc:
        ...
        eps = self.params.epsilon + inflate
        ...
        if feasible:
            used = (best != action)
            return best, {"shield_used": used, "shield_reason": "feasible_alt" if used else "original_ok",
                          "eps": eps, "inflate": inflate, "cur_d": cur_d}
        fallback = self._fallback_action()
        return fallback, {"shield_used": True, "shield_reason": "fallback", "eps": eps, "inflate": inflate, "cur_d": cur_d}
    ...
    def _candidate_actions(self, action) -> List[object]:
        ...
        if a.size < 2:
            return [action]
        steer = float(a[0])
        return [np.array([steer, float(a[1])], dtype=np.float32),
