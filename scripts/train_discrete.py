...
def main():
    ...
    ap.add_argument("--preset", default="", help="Optional preset name (see scripts/presets.py), e.g. merge_discrete_default")
    ...
    if args.preset:
        ...
        # override env if preset specifies
        args.env = preset_cfg.get("env_id", args.env)
        # override nonstationarity
    ...
    env, _, _ = make_env(args.env, args.seed, "discrete", args.p_stay, args.no_mpc, args.no_conformal, safety_params)
    ...
    if args.algo == "dqn":
        model = DQN(
            "MlpPolicy",
            env,
            learning_rate=hp.get("dqn", {}).get("learning_rate", 3e-4),
            buffer_size=hp.get("dqn", {}).get("buffer_size", 100_000),
            learning_starts=hp.get("dqn", {}).get("learning_starts", 10_000),
            batch_size=hp.get("dqn", {}).get("batch_size", 64),
            gamma=hp.get("dqn", {}).get("gamma", 0.99),
            train_freq=hp.get("dqn", {}).get("train_freq", 4),
            target_update_interval=hp.get("dqn", {}).get("target_update_interval", 1_000), verbose=1, seed=args.seed)
    else:
        model = PPO(
            "MlpPolicy",
            env,
            learning_rate=hp.get("ppo", {}).get("learning_rate", 3e-4),
            n_steps=hp.get("ppo", {}).get("n_steps", 2048),
            batch_size=hp.get("ppo", {}).get("batch_size", 64),
            gamma=hp.get("ppo", {}).get("gamma", 0.99),
            gae_lambda=hp.get("ppo", {}).get("gae_lambda", 0.95),
            ent_coef=hp.get("ppo", {}).get("ent_coef", 0.0),
            verbose=1,
            seed=args.seed,
        )
